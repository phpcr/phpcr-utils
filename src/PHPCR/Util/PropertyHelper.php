...
/**
 * Ported from: 
 * http://svn.apache.org/viewvc/jackrabbit/trunk/jackrabbit-jcr-tests/src/main/java/org/apache/jackrabbit/test/api/PropertyUtil.java?view=markup
 *
...
class PropertyHelper
{
     ...
     */
    const LETTER = BASE_CHAR + "|" + IDEOGRAPHIC;

    const NC_NAME =
        "[" + LETTER + "|_]" +
        "[" +  LETTER + "|" + DIGIT + "|.|\\-|_|" + COMBINING_CHAR + "|" + EXTENDER +"]*";

    const SIMPLENAME_CHAR =
        "[^/:\\[\\]\\*'\"\\s]";

    const PATTERNSTRING_NAME =
        "((" + NC_NAME + "):)?" + // prefix
        SIMPLENAME_CHAR + "([" + SIMPLENAME_CHAR + "| ]*" + SIMPLENAME_CHAR +")?";

    const Pattern PATTERN_NAME = Pattern.compile(PATTERNSTRING_NAME);

    const PATTERNSTRING_PATH_ELEMENT =
        PATTERNSTRING_NAME +
        "(\\[[1-9]\\d*\\])?";

    const  PATTERNSTRING_PATH_WITHOUT_LAST_SLASH =
        "(\\./|\\.\\./|/)?" +
        "(" + PropertyUtil.PATTERNSTRING_PATH_ELEMENT + "/)*" +
        PropertyUtil.PATTERNSTRING_PATH_ELEMENT;

    const  PATTERNSTRING_PATH =
        PATTERNSTRING_PATH_WITHOUT_LAST_SLASH + "/?";

    const  PATTERNSTRING_DATE =
     ...
     */
    public static function searchProp(SessionInterface $session, NodeInterface $node, integer $type, boolean $multiple)
    {
        $prop = null;
        ...
        if ($prop === null) {
            foreach ($node->getProperties() as $property) {
                ...
                if ($propType == $type && ($multiple == null || (boolean) $multiple === $property->getDefinition()->isMultiple())) {
                    $prop = $property;
                    break;
                }
            }
        ...
        if ($prop === null) {
            foreach ($nodes as $n) {
                $prop = self::searchProp($session, $n, $type, $multiple);
                if ($prop != null) {
                    break;
                }
            }
     ...
     */
    public static function checkNameFormat(String name, Session session) 
    {
        if ($name == null || strlen($name) == 0) {
            ...
            $matches = (boolean) preg_match('{' . PATTERN_NAME . '}', $name);
     ...
     */
    public static function checkPathFormat($jcrPath, SessionInterface $session)
    {
        if ($jcrPath == null || strlen($jcrPath) == 0) {
            ...
            $elems = explode('/', $jcrPath;

            for ($i = strpos('/') === 1 ? 1 : 0; $i < count($elems); $i++) {
                // validate path element
                $elem = $elems[$i];
                $match = preg_match('{' . PATTERNSTRING_PATH . '}', $elem, $matches);
                if (!$match) {
                    break;
                }
                // validate namespace prefixes if present
    ...
    {
        return preg_match('{' . PATTERNSTRING_DATE . '}', $str);
    }
